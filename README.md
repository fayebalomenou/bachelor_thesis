...
## Setup
To run the code ensure you have **Python 3.10** or higher. The jubyter notebooks can be run using **Jupyter Lab** or **VSCode**.
...
Run the floor-conversions.ipynb notebook to create the individual .ttl files foe each floor.
...
### 3. QLever
Due to size limitations QLever was deployed on a Google Cloud Compute Engine Virtual Machine in order to store and query the Knowledge Graph. The merged_floors.ttl was downloaded to the VM from Google Drive using *gdown* and was indexed in order to be querable. 
...
This notebook is used for deriving the occupancy proxies for MCO and Thingy sensors. The resulted Python Dataframes containing the occupancy labels (df_mco_occupancy.pkl and df_thingy_occupancy.pkl) are located in the **data** folder.
### 6. random_forest.ipynb
This notebooks is used for the Random Forest Analysis. Environmental features are queried separately and merged with the previous created occupancy labels. The resulted Python Dataframes (df_mco_raw.pkl and df_thingy_raw.pkl) are located in the **data** folder.

## Reproducibility 
The code can be executed without regenerating anything as long as the path configuration is  
