...
The required libraries for each notebook are listed inside at the top and can be downloaded from there directly. 
...
The **ENDPOINT** in each of the jupyter notebooks:
...
This notebook is used for deriving the occupancy proxies for MCO and Thingy sensors. The resulting Python Dataframes (df_mco_occupancy.pkl and df_thingy_occupancy.pkl) contain the occupancy labels.
### 6. random_forest.ipynb
This notebook is used for the Random Forest Analysis. Environmental features are queried separately and merged with the previous created occupancy labels. The resulting Python Dataframes (df_mco_raw.pkl and df_thingy_raw.pkl) merge the queried features and occupancy labels.
